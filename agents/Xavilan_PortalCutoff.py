...
class Xavilan(BaseAgent):
    ...
    def init_example_agent(self):
        ...
        self.portalAvg = np.sum(np.max(self.q_table[:,:,0])*self.unexplored)/np.sum(self.unexplored)
    ...
    def select_action(self):
        ...
        action=int(np.argmax(self.q_table[self.state_0][0]\
                         +(self.q_table[self.state_0][1]==0)*(self.q_table[self.state_0][0]<=self.portalAvg)*self.punish/2\
                         -(self.q_table[self.state_0][1]==0)*(self.q_table[self.state_0][0]>self.portalAvg)*self.punish/2\
                         +np.array([self.state_0[0]<self.state_0[1],self.state_0[0]<self.state_0[1],self.state_0[0]>self.state_0[1],self.state_0[0]>self.state_0[1]])*self.punish/4 \
                         +self.punish/8*np.random.rand(self.space_action_n)))
        return action
    ...
    def observe(self, obv, reward, done, action):
        ...
        if actState!=expState: # if actual state is the expected state
            ...
            self.unexplored[actState]=0 # mark as explored
            self.q_table_update()
    ...
    def q_table_update(self):
        ...
        if unexplored==0 or self.portalTotal-self.portalCount==0:
            ...
            self.portalAvg = np.sum(np.max(self.q_table[:,:,0,:],axis=2)*self.unexplored)/np.sum(self.unexplored)
